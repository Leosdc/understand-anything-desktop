...
将任何代码库转换为交互式的 3D 知识图谱，以进行可视化探索、搜索和审计。**现在配备了美观、零本地依赖的 Windows 桌面端应用程序！**
...
* **费用估算与 Token 限制**：在消耗 Gemini 或 Claude API 额度之前，桌面端应用会首先扫描您的代码库文件夹，并显示待分析文件数、预计 AI 请求批次及 Token 估算总结，供您选择继续或取消。**您也可以直接在此弹窗中编辑 `.understandignore` 排除规则，并实时重新计算费用！**
...
* **动态多语言同步**：随时在设置面板切换软件语言。桌面端会自动同步修改当前活动项目的配置文件，使 3D 渲染面板能立即切换至您偏好的语言显示。
...
为了避免不必要的费用：
1. **内置实时编辑器**：您可以在调用 AI 分析前，在费用估算弹窗中直接创建或编辑 `.understandignore` 规则模式。
2. 或者，在您的项目文件夹中，找到或创建 `.understand-anything/` 目录。
3. 在该目录下创建或编辑名为 `.understandignore` 的文件。
4. 添加需要忽略的文件或目录的 glob 匹配模式（例如 `node_modules/`、`dist/`、`.git/`、日志文件或静态图片等）。
5. 欲了解详细的配置规则和高级过滤技巧，请参阅完整的说明文档 [TUTORIAL.zh.md](file:///c:/Users/PC/Documents/Bots/Understand-Anything/docs/TUTORIAL.zh.md)！
...
graph TD
    ...
    Express -->|8. 交互式 3D 图谱呈现| Iframe[嵌入式 Dashboard 视窗]
```
